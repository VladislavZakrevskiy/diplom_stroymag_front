    ...
    <div className="fixed inset-0 bg-gray-800 bg-opacity-50 z-50 md:hidden">
      <div className="absolute right-0 top-0 h-full w-64 bg-white shadow-lg p-4">
        ...
        <nav className="space-y-4">
          <Link href="/products" className={`block py-2 ${pathname.startsWith("/products") ? "font-medium" : ""}`}>
            Каталог
          </Link>
          <Link href="/promotions" className={`block py-2 ${pathname.startsWith("/promotions") ? "font-medium" : ""}`}>
            Акции
          </Link>
          <Link href="/delivery" className={`block py-2 ${pathname.startsWith("/delivery") ? "font-medium" : ""}`}>
            Доставка
          </Link>
          <Link href="/contacts" className={`block py-2 ${pathname.startsWith("/contacts") ? "font-medium" : ""}`}>
            Контакты
          </Link>
